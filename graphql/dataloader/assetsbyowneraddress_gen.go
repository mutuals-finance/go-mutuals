...
type AssetsByOwnerAddressSettings interface {
...
func (l *AssetsByOwnerAddress) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AssetsByOwnerAddress) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AssetsByOwnerAddress) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AssetsByOwnerAddress) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AssetsByOwnerAddress) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *AssetsByOwnerAddress) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *AssetsByOwnerAddress) setPostFetchHook(postFetchHook func(context.Context, string)) {
...
func NewAssetsByOwnerAddress(
...
) *AssetsByOwnerAddress {
	loader := &AssetsByOwnerAddress{
	...
	loader.fetch = func(keys []persist.ChainAddress) ([][]coredb.Asset, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "AssetsByOwnerAddress")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "AssetsByOwnerAddress")
		}
...
// AssetsByOwnerAddress batches and caches requests
type AssetsByOwnerAddress struct {
	...
	batch *assetsByOwnerAddressBatch
...
func (l *AssetsByOwnerAddress) LoadThunk(key persist.ChainAddress) func() ([]coredb.Asset, error) {
	...
	if l.batch == nil {
		l.batch = &assetsByOwnerAddressBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AssetsByOwnerAddress) Clear(key persist.ChainAddress) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (b *assetsByOwnerAddressBatch) keyIndex(l *AssetsByOwnerAddress, key persist.ChainAddress) int {
...
func (b *assetsByOwnerAddressBatch) startTimer(l *AssetsByOwnerAddress) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *assetsByOwnerAddressBatch) end(l *AssetsByOwnerAddress) {
...
func (l *AssetsByOwnerAddress) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
