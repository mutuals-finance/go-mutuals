...
type SplitLoaderByChainAddressSettings interface {
...
// SplitLoaderByChainAddressCacheSubscriptions
type SplitLoaderByChainAddressCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.ChainAddress cache key for a coredb.Split.
	// If AutoCacheWithKey is not nil, this loader will automatically cache published results from other loaders
	// that return a coredb.Split. Loaders that return pointers or slices of coredb.Split
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Split type.
	AutoCacheWithKey func(coredb.Split) persist.ChainAddress

	// AutoCacheWithKeys is a function that returns the []persist.ChainAddress cache keys for a coredb.Split.
	...
	// that return a coredb.Split. Loaders that return pointers or slices of coredb.Split
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Split type.
	AutoCacheWithKeys func(coredb.Split) []persist.ChainAddress
...
func (l *SplitLoaderByChainAddress) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *SplitLoaderByChainAddress) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *SplitLoaderByChainAddress) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *SplitLoaderByChainAddress) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *SplitLoaderByChainAddress) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *SplitLoaderByChainAddress) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *SplitLoaderByChainAddress) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewSplitLoaderByChainAddress creates a new SplitLoaderByChainAddress with the given settings, functions, and options
func NewSplitLoaderByChainAddress(
	settings SplitLoaderByChainAddressSettings, fetch func(ctx context.Context, keys []persist.ChainAddress) ([]coredb.Split, []error),
	funcs SplitLoaderByChainAddressCacheSubscriptions,
	opts ...func(interface {
...
) *SplitLoaderByChainAddress {
	loader := &SplitLoaderByChainAddress{
	...
	loader.fetch = func(keys []persist.ChainAddress) ([]coredb.Split, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "SplitLoaderByChainAddress")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "SplitLoaderByChainAddress")
		}
...
// SplitLoaderByChainAddress batches and caches requests
type SplitLoaderByChainAddress struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []persist.ChainAddress) ([]coredb.Split, []error)
	...
	cache map[persist.ChainAddress]coredb.Split

	// typed cache functions
	//subscribers []func(coredb.Split)
	subscribers []splitLoaderByChainAddressSubscriber
	...
	batch *splitLoaderByChainAddressBatch
...
type splitLoaderByChainAddressBatch struct {
...
func (l *SplitLoaderByChainAddress) Load(key persist.ChainAddress) (coredb.Split, error) {
...
func (l *SplitLoaderByChainAddress) LoadThunk(key persist.ChainAddress) func() (coredb.Split, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &splitLoaderByChainAddressBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *SplitLoaderByChainAddress) LoadAll(keys []persist.ChainAddress) ([]coredb.Split, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SplitLoaderByChainAddress) LoadAllThunk(keys []persist.ChainAddress) func() ([]coredb.Split, []error) {
	results := make([]func() (coredb.Split, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *SplitLoaderByChainAddress) Prime(key persist.ChainAddress, value coredb.Split) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *SplitLoaderByChainAddress) unsafePrime(key persist.ChainAddress, value coredb.Split) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
func (l *SplitLoaderByChainAddress) Clear(key persist.ChainAddress) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *SplitLoaderByChainAddress) unsafeSet(key persist.ChainAddress, value coredb.Split) {
	if l.cache == nil {
		l.cache = map[persist.ChainAddress]coredb.Split{}
	}
	l.cache[key] = value
}
...
func (b *splitLoaderByChainAddressBatch) keyIndex(l *SplitLoaderByChainAddress, key persist.ChainAddress) int {
...
func (b *splitLoaderByChainAddressBatch) startTimer(l *SplitLoaderByChainAddress) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *splitLoaderByChainAddressBatch) end(l *SplitLoaderByChainAddress) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type splitLoaderByChainAddressSubscriber struct {
	cacheFunc func(coredb.Split)
	mutex     *sync.Mutex
}

func (l *SplitLoaderByChainAddress) publishToSubscribers(value coredb.Split) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Split)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, splitLoaderByChainAddressSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *SplitLoaderByChainAddress) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *SplitLoaderByChainAddress) ownsCacheFunc(f *func(coredb.Split)) bool {
