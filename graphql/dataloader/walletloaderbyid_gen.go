...
type WalletLoaderByIdSettings interface {
...
// WalletLoaderByIdCacheSubscriptions
type WalletLoaderByIdCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.DBID cache key for a coredb.Wallet.
	...
	AutoCacheWithKey func(coredb.Wallet) persist.DBID

	// AutoCacheWithKeys is a function that returns the []persist.DBID cache keys for a coredb.Wallet.
	...
	// that return a coredb.Wallet. Loaders that return pointers or slices of coredb.Wallet
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Wallet type.
	AutoCacheWithKeys func(coredb.Wallet) []persist.DBID
...
func (l *WalletLoaderById) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *WalletLoaderById) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *WalletLoaderById) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *WalletLoaderById) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *WalletLoaderById) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *WalletLoaderById) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *WalletLoaderById) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewWalletLoaderById creates a new WalletLoaderById with the given settings, functions, and options
func NewWalletLoaderById(
	settings WalletLoaderByIdSettings, fetch func(ctx context.Context, keys []persist.DBID) ([]coredb.Wallet, []error),
	funcs WalletLoaderByIdCacheSubscriptions,
	opts ...func(interface {
	...
	loader.fetch = func(keys []persist.DBID) ([]coredb.Wallet, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "WalletLoaderById")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "WalletLoaderById")
		}
...
type WalletLoaderById struct {
	...
	subscribers []walletLoaderByIdSubscriber
...
func (l *WalletLoaderById) unsafePrime(key persist.DBID, value coredb.Wallet) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
type walletLoaderByIdSubscriber struct {
	cacheFunc func(coredb.Wallet)
	mutex     *sync.Mutex
}

func (l *WalletLoaderById) publishToSubscribers(value coredb.Wallet) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Wallet)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, walletLoaderByIdSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *WalletLoaderById) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *WalletLoaderById) ownsCacheFunc(f *func(coredb.Wallet)) bool {
