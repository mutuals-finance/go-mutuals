...
type UsersLoaderByContractIDSettings interface {
...
func (l *UsersLoaderByContractID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *UsersLoaderByContractID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *UsersLoaderByContractID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *UsersLoaderByContractID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *UsersLoaderByContractID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *UsersLoaderByContractID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *UsersLoaderByContractID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewUsersLoaderByContractID creates a new UsersLoaderByContractID with the given settings, functions, and options
func NewUsersLoaderByContractID(
	settings UsersLoaderByContractIDSettings, fetch func(ctx context.Context, keys []coredb.GetOwnersByContractIdBatchPaginateParams) ([][]coredb.User, []error),
	opts ...func(interface {
...
) *UsersLoaderByContractID {
	loader := &UsersLoaderByContractID{
	...
	loader.fetch = func(keys []coredb.GetOwnersByContractIdBatchPaginateParams) ([][]coredb.User, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "UsersLoaderByContractID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "UsersLoaderByContractID")
		}
...
// UsersLoaderByContractID batches and caches requests
type UsersLoaderByContractID struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []coredb.GetOwnersByContractIdBatchPaginateParams) ([][]coredb.User, []error)
	...
	cache map[coredb.GetOwnersByContractIdBatchPaginateParams][]coredb.User

	// typed cache functions
	//subscribers []func([]coredb.User)
	subscribers []usersLoaderByContractIDSubscriber
	...
	batch *usersLoaderByContractIDBatch
...
type usersLoaderByContractIDBatch struct {
	keys    []coredb.GetOwnersByContractIdBatchPaginateParams
	data    [][]coredb.User
	error   []error
	closing bool
	done    chan struct{}
...
func (l *UsersLoaderByContractID) Load(key coredb.GetOwnersByContractIdBatchPaginateParams) ([]coredb.User, error) {
...
func (l *UsersLoaderByContractID) LoadThunk(key coredb.GetOwnersByContractIdBatchPaginateParams) func() ([]coredb.User, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &usersLoaderByContractIDBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *UsersLoaderByContractID) LoadAll(keys []coredb.GetOwnersByContractIdBatchPaginateParams) ([][]coredb.User, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UsersLoaderByContractID) LoadAllThunk(keys []coredb.GetOwnersByContractIdBatchPaginateParams) func() ([][]coredb.User, []error) {
	results := make([]func() ([]coredb.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UsersLoaderByContractID) Prime(key coredb.GetOwnersByContractIdBatchPaginateParams, value []coredb.User) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *UsersLoaderByContractID) Clear(key coredb.GetOwnersByContractIdBatchPaginateParams) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *UsersLoaderByContractID) unsafeSet(key coredb.GetOwnersByContractIdBatchPaginateParams, value []coredb.User) {
	if l.cache == nil {
		l.cache = map[coredb.GetOwnersByContractIdBatchPaginateParams][]coredb.User{}
	}
	l.cache[key] = value
}
...
func (b *usersLoaderByContractIDBatch) keyIndex(l *UsersLoaderByContractID, key coredb.GetOwnersByContractIdBatchPaginateParams) int {
...
func (b *usersLoaderByContractIDBatch) startTimer(l *UsersLoaderByContractID) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *usersLoaderByContractIDBatch) end(l *UsersLoaderByContractID) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type usersLoaderByContractIDSubscriber struct {
	cacheFunc func(coredb.User)
	mutex     *sync.Mutex
}

func (l *UsersLoaderByContractID) publishToSubscribers(value []coredb.User) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.User)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, usersLoaderByContractIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *UsersLoaderByContractID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *UsersLoaderByContractID) ownsCacheFunc(f *func(coredb.User)) bool {
