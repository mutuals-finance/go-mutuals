...
type ContractLoaderByIDSettings interface {
...
// ContractLoaderByIDCacheSubscriptions
type ContractLoaderByIDCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.DBID cache key for a coredb.Contract.
	...
	AutoCacheWithKey func(coredb.Contract) persist.DBID

	// AutoCacheWithKeys is a function that returns the []persist.DBID cache keys for a coredb.Contract.
	...
	// that return a coredb.Contract. Loaders that return pointers or slices of coredb.Contract
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Contract type.
	AutoCacheWithKeys func(coredb.Contract) []persist.DBID
...
func (l *ContractLoaderByID) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *ContractLoaderByID) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *ContractLoaderByID) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *ContractLoaderByID) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *ContractLoaderByID) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *ContractLoaderByID) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *ContractLoaderByID) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewContractLoaderByID creates a new ContractLoaderByID with the given settings, functions, and options
func NewContractLoaderByID(
	settings ContractLoaderByIDSettings, fetch func(ctx context.Context, keys []persist.DBID) ([]coredb.Contract, []error),
	funcs ContractLoaderByIDCacheSubscriptions,
	opts ...func(interface {
	...
	loader.fetch = func(keys []persist.DBID) ([]coredb.Contract, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "ContractLoaderByID")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "ContractLoaderByID")
		}
...
type ContractLoaderByID struct {
	...
	subscribers []contractLoaderByIDSubscriber
...
func (l *ContractLoaderByID) unsafePrime(key persist.DBID, value coredb.Contract) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
type contractLoaderByIDSubscriber struct {
	cacheFunc func(coredb.Contract)
	mutex     *sync.Mutex
}

func (l *ContractLoaderByID) publishToSubscribers(value coredb.Contract) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Contract)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, contractLoaderByIDSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *ContractLoaderByID) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *ContractLoaderByID) ownsCacheFunc(f *func(coredb.Contract)) bool {
