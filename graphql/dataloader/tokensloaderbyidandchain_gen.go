...
func NewTokensLoaderByIDAndChain(
	settings TokensLoaderByIDAndChainSettings, fetch func(ctx context.Context, keys []IDAndChain) ([][]Token, []error),
	opts ...func(interface {
	...
	loader.fetch = func(keys []IDAndChain) ([][]Token, []error) {
		ctx := loader.ctx
...
type TokensLoaderByIDAndChain struct {
	...
	fetch func(keys []IDAndChain) ([][]Token, []error)
...
func (l *TokensLoaderByIDAndChain) Load(key IDAndChain) ([]Token, error) {
...
func (l *TokensLoaderByIDAndChain) LoadThunk(key IDAndChain) func() ([]Token, error) {
	l.mu.Lock()
	if !l.disableCaching {
...
func (l *TokensLoaderByIDAndChain) LoadAll(keys []IDAndChain) ([][]Token, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TokensLoaderByIDAndChain) LoadAllThunk(keys []IDAndChain) func() ([][]Token, []error) {
	results := make([]func() ([]Token, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TokensLoaderByIDAndChain) Prime(key IDAndChain, value []Token) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *TokensLoaderByIDAndChain) unsafeSet(key IDAndChain, value []Token) {
	if l.cache == nil {
		l.cache = map[IDAndChain][]Token{}
	}
	l.cache[key] = value
}
...
func (l *TokensLoaderByIDAndChain) publishToSubscribers(value []Token) {
...
func (l *TokensLoaderByIDAndChain) ownsCacheFunc(f *func(Token)) bool {
