...
type AssetsLoaderByChainAddressSettings interface {
...
func (l *AssetsLoaderByChainAddress) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *AssetsLoaderByChainAddress) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *AssetsLoaderByChainAddress) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *AssetsLoaderByChainAddress) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *AssetsLoaderByChainAddress) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *AssetsLoaderByChainAddress) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *AssetsLoaderByChainAddress) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewAssetsLoaderByChainAddress creates a new AssetsLoaderByChainAddress with the given settings, functions, and options
func NewAssetsLoaderByChainAddress(
	settings AssetsLoaderByChainAddressSettings, fetch func(ctx context.Context, keys []persist.ChainAddress) ([][]coredb.Asset, []error),
	opts ...func(interface {
...
) *AssetsLoaderByChainAddress {
	loader := &AssetsLoaderByChainAddress{
	...
	loader.fetch = func(keys []persist.ChainAddress) ([][]coredb.Asset, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "AssetsLoaderByChainAddress")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "AssetsLoaderByChainAddress")
		}
...
// AssetsLoaderByChainAddress batches and caches requests
type AssetsLoaderByChainAddress struct {
	...
	subscribers []assetsLoaderByChainAddressSubscriber
	...
	batch *assetsLoaderByChainAddressBatch
...
type assetsLoaderByChainAddressBatch struct {
...
func (l *AssetsLoaderByChainAddress) Load(key persist.ChainAddress) ([]coredb.Asset, error) {
...
func (l *AssetsLoaderByChainAddress) LoadThunk(key persist.ChainAddress) func() ([]coredb.Asset, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &assetsLoaderByChainAddressBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *AssetsLoaderByChainAddress) LoadAll(keys []persist.ChainAddress) ([][]coredb.Asset, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *AssetsLoaderByChainAddress) LoadAllThunk(keys []persist.ChainAddress) func() ([][]coredb.Asset, []error) {
	results := make([]func() ([]coredb.Asset, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *AssetsLoaderByChainAddress) Prime(key persist.ChainAddress, value []coredb.Asset) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *AssetsLoaderByChainAddress) Clear(key persist.ChainAddress) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *AssetsLoaderByChainAddress) unsafeSet(key persist.ChainAddress, value []coredb.Asset) {
	if l.cache == nil {
...
func (b *assetsLoaderByChainAddressBatch) keyIndex(l *AssetsLoaderByChainAddress, key persist.ChainAddress) int {
...
func (b *assetsLoaderByChainAddressBatch) startTimer(l *AssetsLoaderByChainAddress) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *assetsLoaderByChainAddressBatch) end(l *AssetsLoaderByChainAddress) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type assetsLoaderByChainAddressSubscriber struct {
	cacheFunc func(coredb.Asset)
	mutex     *sync.Mutex
}

func (l *AssetsLoaderByChainAddress) publishToSubscribers(value []coredb.Asset) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Asset)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, assetsLoaderByChainAddressSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *AssetsLoaderByChainAddress) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *AssetsLoaderByChainAddress) ownsCacheFunc(f *func(coredb.Asset)) bool {
