...
type ContractLoaderByChainAddressSettings interface {
...
// ContractLoaderByChainAddressCacheSubscriptions
type ContractLoaderByChainAddressCacheSubscriptions struct {
	// AutoCacheWithKey is a function that returns the persist.ChainAddress cache key for a coredb.Contract.
	// If AutoCacheWithKey is not nil, this loader will automatically cache published results from other loaders
	// that return a coredb.Contract. Loaders that return pointers or slices of coredb.Contract
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Contract type.
	AutoCacheWithKey func(coredb.Contract) persist.ChainAddress

	// AutoCacheWithKeys is a function that returns the []persist.ChainAddress cache keys for a coredb.Contract.
	...
	// that return a coredb.Contract. Loaders that return pointers or slices of coredb.Contract
	// will be dereferenced/iterated automatically, invoking this function with the base coredb.Contract type.
	AutoCacheWithKeys func(coredb.Contract) []persist.ChainAddress
...
func (l *ContractLoaderByChainAddress) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *ContractLoaderByChainAddress) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *ContractLoaderByChainAddress) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *ContractLoaderByChainAddress) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *ContractLoaderByChainAddress) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *ContractLoaderByChainAddress) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *ContractLoaderByChainAddress) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewContractLoaderByChainAddress creates a new ContractLoaderByChainAddress with the given settings, functions, and options
func NewContractLoaderByChainAddress(
	settings ContractLoaderByChainAddressSettings, fetch func(ctx context.Context, keys []persist.ChainAddress) ([]coredb.Contract, []error),
	funcs ContractLoaderByChainAddressCacheSubscriptions,
	opts ...func(interface {
...
) *ContractLoaderByChainAddress {
	loader := &ContractLoaderByChainAddress{
	...
	loader.fetch = func(keys []persist.ChainAddress) ([]coredb.Contract, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "ContractLoaderByChainAddress")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "ContractLoaderByChainAddress")
		}
	...
	if !loader.disableCaching {
		...
		if funcs.AutoCacheWithKey != nil {
			cacheFunc := func(t coredb.Contract) {
...
type ContractLoaderByChainAddress struct {
	...
	subscribers []contractLoaderByChainAddressSubscriber
...
func (l *ContractLoaderByChainAddress) unsafePrime(key persist.ChainAddress, value coredb.Contract) bool {
	if l.disableCaching {
		return false
	}
	var found bool
...
type contractLoaderByChainAddressSubscriber struct {
	cacheFunc func(coredb.Contract)
	mutex     *sync.Mutex
}

func (l *ContractLoaderByChainAddress) publishToSubscribers(value coredb.Contract) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Contract)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, contractLoaderByChainAddressSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *ContractLoaderByChainAddress) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *ContractLoaderByChainAddress) ownsCacheFunc(f *func(coredb.Contract)) bool {
