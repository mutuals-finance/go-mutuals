...
type TokensLoaderByIDAndLimitSettings interface {
...
func (l *TokensLoaderByIDAndLimit) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *TokensLoaderByIDAndLimit) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *TokensLoaderByIDAndLimit) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *TokensLoaderByIDAndLimit) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *TokensLoaderByIDAndLimit) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *TokensLoaderByIDAndLimit) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *TokensLoaderByIDAndLimit) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewTokensLoaderByIDAndLimit creates a new TokensLoaderByIDAndLimit with the given settings, functions, and options
func NewTokensLoaderByIDAndLimit(
	settings TokensLoaderByIDAndLimitSettings, fetch func(ctx context.Context, keys []IDAndLimit) ([][]coredb.Token, []error),
	opts ...func(interface {
...
) *TokensLoaderByIDAndLimit {
	loader := &TokensLoaderByIDAndLimit{
	...
	loader.fetch = func(keys []IDAndLimit) ([][]coredb.Token, []error) {
		ctx := loader.ctx

		// Allow the preFetchHook to modify and return a new context
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "TokensLoaderByIDAndLimit")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "TokensLoaderByIDAndLimit")
		}
...
// TokensLoaderByIDAndLimit batches and caches requests
type TokensLoaderByIDAndLimit struct {
	// context passed to fetch functions
	ctx context.Context

	// this method provides the data for the loader
	fetch func(keys []IDAndLimit) ([][]coredb.Token, []error)
	...
	cache map[IDAndLimit][]coredb.Token

	// typed cache functions
	//subscribers []func([]coredb.Token)
	subscribers []tokensLoaderByIDAndLimitSubscriber
	...
	batch *tokensLoaderByIDAndLimitBatch
...
type tokensLoaderByIDAndLimitBatch struct {
	keys    []IDAndLimit
	data    [][]coredb.Token
	error   []error
	closing bool
...
func (l *TokensLoaderByIDAndLimit) Load(key IDAndLimit) ([]coredb.Token, error) {
...
func (l *TokensLoaderByIDAndLimit) LoadThunk(key IDAndLimit) func() ([]coredb.Token, error) {
	l.mu.Lock()
	if !l.disableCaching {
	...
	if l.batch == nil {
		l.batch = &tokensLoaderByIDAndLimitBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *TokensLoaderByIDAndLimit) LoadAll(keys []IDAndLimit) ([][]coredb.Token, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TokensLoaderByIDAndLimit) LoadAllThunk(keys []IDAndLimit) func() ([][]coredb.Token, []error) {
	results := make([]func() ([]coredb.Token, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TokensLoaderByIDAndLimit) Prime(key IDAndLimit, value []coredb.Token) bool {
	if l.disableCaching {
		return false
	}
	l.mu.Lock()
...
func (l *TokensLoaderByIDAndLimit) Clear(key IDAndLimit) {
	if l.disableCaching {
		return
	}
	l.mu.Lock()
...
func (l *TokensLoaderByIDAndLimit) unsafeSet(key IDAndLimit, value []coredb.Token) {
	if l.cache == nil {
		l.cache = map[IDAndLimit][]coredb.Token{}
	}
	l.cache[key] = value
}
...
func (b *tokensLoaderByIDAndLimitBatch) keyIndex(l *TokensLoaderByIDAndLimit, key IDAndLimit) int {
...
func (b *tokensLoaderByIDAndLimitBatch) startTimer(l *TokensLoaderByIDAndLimit) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *tokensLoaderByIDAndLimitBatch) end(l *TokensLoaderByIDAndLimit) {
	b.data, b.error = l.fetch(b.keys)
	close(b.done)
}

type tokensLoaderByIDAndLimitSubscriber struct {
	cacheFunc func(coredb.Token)
	mutex     *sync.Mutex
}

func (l *TokensLoaderByIDAndLimit) publishToSubscribers(value []coredb.Token) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.Token)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, tokensLoaderByIDAndLimitSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *TokensLoaderByIDAndLimit) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *TokensLoaderByIDAndLimit) ownsCacheFunc(f *func(coredb.Token)) bool {
