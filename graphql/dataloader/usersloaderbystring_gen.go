...
type UsersLoaderByStringSettings interface {
...
func (l *UsersLoaderByString) setContext(ctx context.Context) {
	l.ctx = ctx
}

func (l *UsersLoaderByString) setWait(wait time.Duration) {
	l.wait = wait
}

func (l *UsersLoaderByString) setMaxBatch(maxBatch int) {
	l.maxBatch = maxBatch
}

func (l *UsersLoaderByString) setDisableCaching(disableCaching bool) {
	l.disableCaching = disableCaching
}

func (l *UsersLoaderByString) setPublishResults(publishResults bool) {
	l.publishResults = publishResults
}

func (l *UsersLoaderByString) setPreFetchHook(preFetchHook func(context.Context, string) context.Context) {
	l.preFetchHook = preFetchHook
}

func (l *UsersLoaderByString) setPostFetchHook(postFetchHook func(context.Context, string)) {
	l.postFetchHook = postFetchHook
}

// NewUsersLoaderByString creates a new UsersLoaderByString with the given settings, functions, and options
func NewUsersLoaderByString(
	settings UsersLoaderByStringSettings, fetch func(ctx context.Context, keys []string) ([][]coredb.User, []error),
	opts ...func(interface {
	...
	loader.fetch = func(keys []string) ([][]coredb.User, []error) {
		...
		if loader.preFetchHook != nil {
			ctx = loader.preFetchHook(ctx, "UsersLoaderByString")
		}

		results, errors := fetch(ctx, keys)

		if loader.postFetchHook != nil {
			loader.postFetchHook(ctx, "UsersLoaderByString")
		}
...
type UsersLoaderByString struct {
	...
	subscribers []usersLoaderByStringSubscriber
...
type usersLoaderByStringSubscriber struct {
	cacheFunc func(coredb.User)
	mutex     *sync.Mutex
}

func (l *UsersLoaderByString) publishToSubscribers(value []coredb.User) {
		...
		for i, subscription := range *l.subscriptionRegistry {
			if typedFunc, ok := subscription.(*func(coredb.User)); ok {
				...
				if !l.ownsCacheFunc(typedFunc) {
					l.subscribers = append(l.subscribers, usersLoaderByStringSubscriber{cacheFunc: *typedFunc, mutex: (*l.mutexRegistry)[i]})
				}
			}
		}
	})
...
func (l *UsersLoaderByString) registerCacheFunc(cacheFunc interface{}, mutex *sync.Mutex) {
...
func (l *UsersLoaderByString) ownsCacheFunc(f *func(coredb.User)) bool {
