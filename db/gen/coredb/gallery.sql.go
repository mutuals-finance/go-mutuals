...
const galleryRepoAddCollections = `-- name: SplitRepoAddCollections :execrows
update splits set last_updated = now(), collections = $1::text[] || collections where splits.id = $2 and (select count(*) from collections c where c.id = any($1) and c.gallery_id = $2 and c.deleted = false) = coalesce(array_length($1, 1), 0)
`
...
update splits set deleted = true where splits.id = $1 and (select count(*) from splits g where g.owner_user_id = $2 and g.deleted = false and not g.id = $1) > 0 and not coalesce((select featured_gallery::varchar from users u where u.id = $2), '') = $1
`
...
update splits set last_updated = now(), collections = $1 where splits.id = $2 and (select count(*) from collections c where c.id = any($1) and c.gallery_id = $2 and c.deleted = false) = coalesce(array_length($1, 1), 0)
`
