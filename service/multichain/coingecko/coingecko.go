...
func init() {
	env.RegisterValidation("COINGECKO_API_KEY", "required")
}
...
var (
	baseURL, _                = url.Parse("https://api.coingecko.com/api/v3")
	getTokensEndpointTemplate = fmt.Sprintf("%s/%s", baseURL.String(), "coins/%s/contract/%s")
)
...
var chainToIdentifier = map[persist.Chain]string{
	...
	persist.ChainPolygon:     "polygon-pos",
	persist.ChainOptimism:    "optimistic-ethereum",
	persist.ChainArbitrum:    "arbitrum-nova",
	persist.ChainBase:        "base",
	persist.ChainBaseSepolia: "", // no testnet support by coingecko
}

type ErrCoingeckoRateLimited struct{ Err error }

func (e ErrCoingeckoRateLimited) Unwrap() error { return e.Err }
func (e ErrCoingeckoRateLimited) Error() string {
...
// GetTokensIncrementallyByContractAddress returns a list of tokens for a contract address
...
// GetTokensByTokenChainAddresses returns a list of tokens for a list of token identifiers
func (p *Provider) GetTokensByTokenChainAddresses(ctx context.Context, ti multichain.ChainAgnosticIdentifiers) ([]multichain.ChainAgnosticToken, multichain.ChainAgnosticContract, error) {
	outCh := make(chan assetsReceived)
	go func() {
		defer close(outCh)
...
func (p *Provider) assetToChainAgnosticTokens(ownerAddress persist.Address, asset Asset) ([]multichain.ChainAgnosticToken, error) {
...
func mustTokensByContractEndpoint(chain persist.Chain, address persist.Address) *url.URL {
	s := fmt.Sprintf(getTokensEndpointTemplate, mustChainIdentifierFrom(chain), address)
	return checkURL(s)
}
...
func streamAssetsForContract(ctx context.Context, r *retry.Retryer, chain persist.Chain, address persist.Address, outCh chan assetsReceived) {
	endpoint := mustTokensByContractEndpoint(chain, address)
	setPagingParams(endpoint)
	paginateAssets(ctx, r, mustAuthRequest(ctx, endpoint), outCh)
}
...
func wrapRateLimitErr(ctx context.Context, err error) error {
	...
	err = ErrCoingeckoRateLimited{err}
