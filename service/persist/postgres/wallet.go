...
func (w *WalletRepository) GetByID(ctx context.Context, ID persist.DBID) (persist.Wallet, error) {
	...
	err := w.getByIDStmt.QueryRowContext(ctx, ID).Scan(&wallet.ID, &wallet.Version, &wallet.CreationTime, &wallet.UpdatedAt, &wallet.Address, &wallet.WalletType, &wallet.Chain)
	if err != nil {
...
func (w *WalletRepository) GetByChainAddress(ctx context.Context, chainAddress persist.L1ChainAddress) (persist.Wallet, error) {
	var wallet persist.Wallet
	err := w.getByChainAddressStmt.QueryRowContext(ctx, chainAddress.Address(), chainAddress.L1Chain()).Scan(&wallet.ID, &wallet.Version, &wallet.CreationTime, &wallet.UpdatedAt, &wallet.Address, &wallet.WalletType, &wallet.Chain, &wallet.L1Chain)
	if err != nil {
...
func (w *WalletRepository) GetByUserID(ctx context.Context, userID persist.DBID) ([]persist.Wallet, error) {
	...
	for rows.Next() {
		var wallet persist.Wallet
		err = rows.Scan(&wallet.ID, &wallet.Version, &wallet.CreationTime, &wallet.UpdatedAt, &wallet.Address, &wallet.WalletType, &wallet.Chain)
		if err != nil {
			return nil, err
		}
