...
func NewContractSplitRepository(db *sql.DB, queries *db.Queries) *ContractSplitRepository {
	...
	upsertByAddressStmt, err := db.PrepareContext(ctx, `INSERT INTO contracts (ID,VERSION,ADDRESS,SYMBOL,NAME,CREATOR_ADDRESS,CHAIN) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (ADDRESS,CHAIN) DO UPDATE SET VERSION = $2, ADDRESS = $3, SYMBOL = $4, CREATOR_ADDRESS = $6, CHAIN = $7;`)
	checkNoErr(err)
