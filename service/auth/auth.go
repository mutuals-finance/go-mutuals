...
var ErrEmailUnverified = errors.New("the email address you provided is unverified. Login with QR code instead, or verify your email at splitfi.so/settings")
...
func (e NonceAuthenticator) Authenticate(pCtx context.Context) (*AuthResult, error) {
	chainAddress := e.ChainPubKey.ToChainAddress()
	nonce, user, _ := GetUserWithNonce(pCtx, chainAddress, e.UserRepo, e.NonceRepo, e.WalletRepo)
	if nonce == "" {
		return nil, ErrNonceNotFound{ChainAddress: chainAddress}
	}
	...
	err = NonceRotate(pCtx, chainAddress, e.NonceRepo)
	if err != nil {
		return nil, err
	}

	walletID := persist.DBID("")
	wallet, err := e.WalletRepo.GetByChainAddress(pCtx, chainAddress)
	if err != nil {
		walletID = wallet.ID
	}

	authResult := AuthResult{
		Addresses: []AuthenticatedAddress{{ChainAddress: chainAddress, WalletType: e.WalletType, WalletID: walletID}},
		User:      user,
	}
